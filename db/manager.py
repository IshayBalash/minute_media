...
class DBManager:
    ...
    def execute_sql_file(self, sql_file_path: str) -> None:
        """Execute a .sql file (DDL, INSERT, UPDATE, DELETE). Supports multiple statements."""
        self._require_connection()
        path = Path(sql_file_path)
        if not path.exists():
            raise FileNotFoundError(f"SQL file not found: {sql_file_path}")
        self.conn.executescript(path.read_text())
        logger.info(f"Executed: {path.name}")

    def execute_sql_string(self, sql: str, params: dict = {}) -> sqlite3.Cursor:
        """Execute a single SQL string (INSERT, UPDATE, DELETE, or DDL). Supports :param_name placeholders."""
        self._require_connection()
        cursor = self.conn.execute(sql, params)
        self.conn.commit()
        return cursor

    def run_sql_file(self, sql_file_path: str, params: dict = {}) -> None:
        """Execute a single-statement .sql file with named parameters (:param_name syntax).
        Use this for pipeline queries (INSERT, UPDATE). For schema/DDL use execute_sql_file."""
        self._require_connection()
        path = Path(sql_file_path)
        if not path.exists():
            raise FileNotFoundError(f"SQL file not found: {sql_file_path}")
    ...
    def query_from_sql_file(self, sql_file_path: str, params: dict = {}) -> pd.DataFrame:
        """Read a .sql file and return results as a DataFrame. Supports :param_name placeholders."""
        self._require_connection()
        path = Path(sql_file_path)
        if not path.exists():
            raise FileNotFoundError(f"SQL file not found: {sql_file_path}")
    ...
    def query_from_sql_string(self, sql: str, params: dict = {}) -> pd.DataFrame:
        """Run a SQL string and return results as a DataFrame. Supports :param_name placeholders."""
        self._require_connection()
